...
from tagging import tagger, tei
...
class TaggerLemmatizer(Processor):
    ...
    def tag_and_lemmatize_sentence_action(self):
        def process(s):
            if self.just_tag:
                self.tagger.tag_tokenized_sentence(
                    s, choose_compatible=self.choose_compatible
                )
            else:
                self.lemmatizer.lemmatize_tokenized_sentence(
                    self.tagger.tag_tokenized_sentence(
                        s, choose_compatible=self.choose_compatible
                    )
                )
    ...
    def init(self):
        ...
        try:
            json_file = pathlib.Path(self.config_path).open()
            config = json.load(json_file)
            if "lem_tokenizer" not in config:
                config["lem_tokenizer"] = config["lem_model"]
            tag_mapping_path = (
                config["tag_mapping_path"] if "tag_mapping_path" in config else None
            )
            chop_pos_to_main = (
                config["chop_pos_to_main"] if "chop_pos_to_main" in config else True
            )
            self.lemmatizer = Lemmatizer(
                config["lem_model"],
                config["lem_tokenizer"],
                config["lexicon_path"],
                tag_mapping_path,
                chop_pos_to_main=chop_pos_to_main,
            )
            self.lemmatizer.init()

            self.choose_compatible = (
                config["choose_compatible"] if "choose_compatible" in config else False
            )
            self.just_tag = config["just_tag"] if "just_tag" in config else False
            tagging_model_name = config["tagging_model"]
            self.tagger = tagger.find_model(
                tagging_model_name, return_alternatives=self.choose_compatible
            )
    ...
    def tagPlainText(self, text, output):
        ...
        lemmatized = list(tqdm(map(a, pos_results)))
        # print(f"lemmatized={lemmatized}")
        self.printSentences(lemmatized, output)

    def process(self, input_filename, output_filename):
        with pathlib.Path(input_filename).open(
    ...
    def printSentences(self, sentences, output):
        f = pathlib.Path(output).open("w+")
        f.write("token\tpos\tlemma\n")
        for s in sentences:
            for token in s:
                if self.verbose:
                    f.write(
                        f"{token['word']}\t{token['pos']}\t{token['lemma']}\t{token['lexiconMatch']}\t{token['info']}\n"
                    )
                else:
    ...
    def tagTSV(
        self, input_path, output_path, fields: dict = None
    ):  # assumes that 'word' is the first property
    ...
    def tagTEI(self, input_path, output_path):
        if pathlib.Path(input_path).is_file():
            tei.doWithTEISentences(
                input_path,
                output_path,
                self.tag_and_lemmatize_sentence_action(),
                also_keep_old=self.choose_compatible,
            )
