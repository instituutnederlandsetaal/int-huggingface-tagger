...
try:
    ...
    from token_properties import connect_linked_tokens
except:
    from .lexicon_databases import Lexicon
    from .token_properties import connect_linked_tokens
...
class Lemmatizer:
    ...
    def lemmatize_byt5(self, token: dict):
        ...
        if wp in self.cache:
            ...
            lemma = re.sub(r"<pad>|</s>", "", self.tokenizer.decode(outputs[0]))
            self.cache[wp] = lemma
    ...
    def prepare_token(self, token: dict):
        wordform = (
            token["grouped_wordform"] if "grouped_wordform" in token else token["word"]
        )
        if "token" in token:
            token["word"] = token["token"]
        if "pos" in token:
            token["pos_head"] = re.sub(r"\(.*", "", self.lexicon.map_pos(token["pos"]))
    ...
    def special_cases(self, token):
        if (token["pos_head"] == "NUM") and re.match(
            r".*[0-9].*",
            token["word"],
        ):  # TODO regels voor telwoorden implementeren!
            token["lemma"] = token["word"]
            # print(str(token),file=sys.stderr)
            return True
        if (token["pos_head"] == "PD") and re.match(r"[Mm][Ee][Nn]", token["word"]):
            token["lemma"] = "men"
            return True
        return False
    ...
    def lemmatize_token(self, token: dict):
        ...
        p = token["pos"]
        if re.match(r".*VRB.*_.A.*", p):  # vreselijk tijdelijk hackje ....
            p = "ADV"
        (compatible_lexical_items, incompatible_lexical_items, info) = (
            self.lexicon.lookup_compatible(wordform, p)
        )

        token["info"] = info

        if token.get("sentence_bound"):
            return
        ...
        if len(compatible_lexical_items) > 0:
            ...
            lemma = token["lemma"]
            # print(str(token),file=sys.stderr)
        elif token["pos"] != "LET" and token["pos"] != "PC":
